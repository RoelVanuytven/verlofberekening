// Debounce functie om veelvuldige opslagoperaties te beperken
let saveTimeout = null;
function debouncedSave() {
    if (saveTimeout) {
        clearTimeout(saveTimeout);
    }
    saveTimeout = setTimeout(saveToSessionStorage, 500);
}
...
function showCookieConsent() {
    try {
        ...
        console.error('Fout bij tonen cookie melding:', error);
    }
}
...
function validateInput(input) {
    try {
        ...
        // Gebruik debounce voor opslaan
        debouncedSave();
    } catch (error) {
        console.error('Fout bij valideren input:', error);
    }
}
...
function validateOpgenomenUren(input, maand) {
    try {
        ...
        console.error('Fout bij valideren opgenomen uren:', error);
    }
}
...
function saveToSessionStorage() {
    try {
        // Verzamel alle gegevens in één object
        const data = {
            percentages: Array.from(document.querySelectorAll('.table-container:first-of-type .percentage-input')).map(input => input.value),
            verlofPercentages: Array.from(document.querySelectorAll('#verlof-tabel .percentage-input:not(.opgenomen-uren)')).map(input => input.value),
            opgenomenUren: Array.from(document.querySelectorAll('.opgenomen-uren')).map(input => input.value),
            advOpgenomenUren: Array.from(document.querySelectorAll('.adv-opgenomen-uren')).map(input => input.value),
            elOpgenomenUren: Array.from(document.querySelectorAll('.el-opgenomen-uren')).map(input => input.value),
            ancOpgenomenUren: Array.from(document.querySelectorAll('.anc-opgenomen-uren')).map(input => input.value),
            startDate: document.getElementById('startDate').value
        };
        
        // Sla alles op in één keer
        sessionStorage.setItem('verlofData', JSON.stringify(data));
        
        // Sla de cookie consent apart op
...
function loadFromSessionStorage() {
    try {
        // Laad alle gegevens in één keer
        const dataString = sessionStorage.getItem('verlofData');
        if (!dataString) {
            setDefaultDate();
            return;
        }
        ...
        // Vul de velden in met de opgeslagen gegevens
        if (data.percentages) {
    ...
    try {
        ...
        inputs.forEach(input => {
            ...
            if (!isNaN(value)) {
                ...
                total += 100; // Standaardwaarde voor ongeldige invoer
            }
        });
        ...
        console.error('Fout bij berekenen gemiddelde:', error);
        return 100; // Standaardwaarde bij fout
    }
}
...
function updateBegrensdWettelijkVerlof(averagePercentage) {
    try {
        ...
        console.error('Fout bij bijwerken begrensd wettelijk verlof:', error);
    }
}
...
function updateTable() {
    try {
        ...
        console.error('Fout bij bijwerken tabel:', error);
    }
}
...
function updateADVTable() {
    try {
        ...
        console.error('Fout bij bijwerken ADV tabel:', error);
    }
}
...
function updateELTable() {
    try {
        ...
        console.error('Fout bij bijwerken EL tabel:', error);
    }
}
...
function updateANCTable() {
    try {
        ...
        console.error('Fout bij bijwerken ANC tabel:', error);
    }
}
...
function setDefaultDate() {
    try {
        ...
        console.error('Fout bij instellen standaarddatum:', error);
    }
}
...
function berekenAncienniteitsverlof() {
    try {
        ...
        console.error('Fout bij berekenen ancienniteitsverlof:', error);
        return 24; // Standaardwaarde bij fout
    }
}
...
function updateStartDate() {
    try {
        ...
        console.error('Fout bij bijwerken startdatum:', error);
    }
}
...
function exportToCSV() {
    try {
        ...
        alert('Er is een probleem opgetreden bij het exporteren. Probeer het opnieuw of neem contact op met de beheerder.');
    }
}
...
function importFromCSV() {
    try {
        ...
        alert('Er is een probleem opgetreden bij het importeren. Probeer het opnieuw of neem contact op met de beheerder.');
    }
}
    ...
    try {
        ...
        reader.onload = function(event) {
            try {
                ...
                console.error('Fout bij verwerken van geïmporteerd bestand:', error);
                alert('Er is een probleem opgetreden bij het verwerken van het bestand. Controleer of het bestand het juiste formaat heeft.');
            }
        };

        reader.readAsText(file);
    } catch (error) {
        console.error('Fout bij bestandsimport:', error);
        alert('Er is een probleem opgetreden bij het lezen van het bestand.');
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    try {
        ...
        if (startDateElem) {
            startDateElem.addEventListener('change', updateStartDate);
        }

        // Bereken alles bij het laden
        calculateAverage();
        updateTable();
    } catch (error) {
        console.error('Fout bij initialisatie:', error);
        // Probeer de essentiële functies toch uit te voeren
        try {
            setDefaultDate();
            calculateAverage();
            updateTable();
        } catch (innerError) {
            console.error('Kritieke fout bij initialisatie:', innerError);
        }
    }
});
